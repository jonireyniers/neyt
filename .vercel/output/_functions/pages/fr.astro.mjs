...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "BaseLayout", $$BaseLayout, { "title": "Neyt Meubles - Meubles de qualit\xE9", "description": "Neyt NV fournit des meubles de qualit\xE9 pour chambres, salles \xE0 manger et bureaux. Fabricant de meubles B2B de Beervelde, Belgique.", "locale": locale }, { "default": async ($$result2) => renderTemplate` ${renderComponent($$result2, "Header", $$Header, { "locale": locale })} ${maybeRenderHead()}<main> <section class="relative h-[350px] md:h-[450px] bg-gradient-to-br from-primary to-secondary overflow-hidden"> <div class="absolute inset-0 bg-black/20"></div> <div class="relative container h-full flex flex-col justify-center items-center text-center text-white"> <img src="/images/furniture/prodlogo.png" alt="Neyt Meubles" class="h-40 md:h-56 lg:h-64 w-auto mb-4 animate-fade-in"> <p class="text-base md:text-lg lg:text-xl mb-5 max-w-3xl font-light"> ${companyData.company.taglines[locale]} </p> <div class="flex gap-4"> <a href="/fr/furniture" class="inline-block bg-white text-primary px-8 py-4 rounded-lg font-semibold hover:bg-gray-100 transition-all transform hover:scale-105 shadow-lg">
Voir Collection
</a> <a href="/fr/about" class="inline-block bg-transparent border-2 border-white text-white px-8 py-4 rounded-lg font-semibold hover:bg-white hover:text-primary transition-all">
À Propos
</a> </div> </div> </section> <section class="section bg-gray-50"> <div class="container"> <div class="text-center mb-12"> <h2 class="text-4xl md:text-5xl font-serif font-bold mb-4">Derniers ajouts</h2> <p class="text-gray-600 text-lg">Découvrez nos derniers meubles</p> </div> <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8"> ${recentFurniture.map((item) => {
  ...
  })} </div> <div class="text-center mt-12"> <a href="/fr/furniture" class="inline-block bg-accent text-white px-8 py-3 rounded-lg font-semibold hover:bg-accent/90 transition-colors">
...
Dates de Fermeture ${companyData.closingDates[0].year} </h3> <ul class="space-y-3"> ${companyData.closingDates[0].periods.map((period) => renderTemplate`<li class="flex items-start gap-3"> <span class="text-accent text-xl mt-0.5">•</span> <span class="text-gray-700">${period}</span> </li>`)} </ul> </div> </div> </section>`} <section class="section bg-gradient-to-br from-primary to-secondary text-white"> <div class="container text-center"> <h2 class="text-3xl md:text-4xl font-serif font-bold mb-4">
Intéressé par nos produits?
</h2> <a href="/fr/contact" class="inline-block bg-white text-primary px-8 py-4 rounded-lg font-semibold hover:bg-gray-100 transition-all transform hover:scale-105 shadow-lg">
Nous contacter
</a> </div> </section> </main> ${renderComponent($$result2, "Footer", $$Footer, { "locale": locale })} ` })}`;
}, "/Users/jonireyniers/prod-neyt/neyt/src/pages/fr/index.astro", void 0);

const $$file = "/Users/jonireyniers/prod-neyt/neyt/src/pages/fr/index.astro";
const $$url = "/fr";
